...
def local_load_val_dataset(name, load_split):
    assert load_split == 'validation'
    # file = './src/dataset/h-swag/haha.jsonl'
    file = os.path.join('./src/dataset', name, 'val.jsonl')
    print('local_load_val:', file)
    huggingFace_data = load_dataset('json', data_files=file, split='train')
...
class DatasetReader(object):
    ...
    def _read_origin_dataset(self, split):
        ...
        if split not in self.cached_origData:
            ...
            # print("*self.dataset_stash------------------------------------------------")
            print(*self.dataset_stash)
            # print("load_split------------------------------------------------")
            print(load_split)
            if self.dataset_stash!="hellaswag":
                huggingFace_data = local_load_val_dataset(self.name, load_split)
                # huggingFace_data = load_dataset(*self.dataset_stash, split=load_split,)
            else:
                huggingFace_data = load_dataset('json', data_files='./src/dataset/h-swag/haha.jsonl', split="train")
    ...
    def _applyTemplate_toData(
        ...
        for datapoint_idx, datapoint in enumerate(orig_data):
            ...
            input_txt, target_txt = template.apply(datapoint if 'ctx_a' in datapoint else {**datapoint, 'ctx_a': 'default_value','ctx_c': 'default_value','ctx_b': 'default_value','ctx_d': 'default_value','endings': 'default_value'})
...
class RTEReader(DatasetReader):
    def __init__(self, dataset_kwargs=None):
        ...
        super().__init__(
            ...
            # dataset_stash=("src/dataset/rte/config.json"), template_stash=("super_glue", "rte") 
        )
...
class COPAReader(DatasetReader):
    def __init__(self, dataset_kwargs=None):
        ...
        super().__init__(
            # dataset_stash=("src/dataset/copa/config.json"), template_stash=("super_glue", "copa")
...
class WinograndeReader(DatasetReader):
    def __init__(self, dataset_kwargs=None):
        ...
        super().__init__(
            # dataset_stash=("src/dataset/winogrande/dev.jsonl"),
    ...
    def _read_origin_dataset(self, split):
        ...
        if split not in self.cached_origData:
            ...
            #      "json",data_files="src/dataset/winogrande/dev.jsonl",
            #     split="validation",
            # )
            huggingFace_data = load_dataset(
                *self.dataset_stash,
...
class StoryClozeReader(DatasetReader):
    def __init__(self, dataset_kwargs=None):
        ...
        super().__init__(
            dataset_stash=("src/dataset/story_cloze/story_cloze.py"),
    ...
    def _read_origin_dataset(self, split):
        ...
        if split not in self.cached_origData:
            ...
            huggingFace_data = load_dataset(
                "src/dataset/story_cloze/story_cloze.py",
                split= "validation",
                data_dir="",
            )
...
class ANLIR1Reader(DatasetReader):
    def __init__(self, dataset_kwargs=None):
        ...
        # super().__init__(dataset_stash=("src/dataset/anli-r1/config.json",), template_stash=("anli",))
    ...
    def _read_origin_dataset(self, split):
        ...
        if split not in self.cached_origData:
            huggingFace_data = load_dataset(
                "json",
                data_files="src/dataset/anli-r1/dev.jsonl",
                split=f"train",
            )
            ...
            for idx, example in enumerate(huggingFace_data):
                example["idx"] = idx
                 # example["lbl"] = int(example["label"])
                label_mapping = {'c': 0, 'e': 1, 'n': 2,}  # Example mapping
                example["lbl"] = label_mapping[example["label"]]
                orig_data.append(example)
...
class ANLIR2Reader(DatasetReader):
    def __init__(self, dataset_kwargs=None):
        ...
        # super().__init__(dataset_stash=("src/dataset/anli-r2/config.json",), template_stash=("anli",))
        super().__init__(dataset_stash=("anli2",), template_stash=("anli",))
        if dataset_kwargs:
            for k, v in dataset_kwargs.items():
                setattr(self, k, v)
    ...
    def _read_origin_dataset(self, split):
        ...
        if split not in self.cached_origData:
            huggingFace_data = load_dataset(
                "json",data_files="src/dataset/anli-r2/dev.jsonl",
                split=f"train",
            )
...
class ANLIR3Reader(DatasetReader):
    def __init__(self, dataset_kwargs=None):
        ...
        # super().__init__(dataset_stash=("src/dataset/anli-r3/config.json",), template_stash=("anli",))
        super().__init__(dataset_stash=("src/dataset/anli-r3/dev.jsonl",), template_stash=("anli",))
        if dataset_kwargs:
            for k, v in dataset_kwargs.items():
                setattr(self, k, v)
    ...
    def _read_origin_dataset(self, split):
        ...
        if split not in self.cached_origData:
            huggingFace_data = load_dataset(
                "json",data_files="src/dataset/anli-r3/dev.jsonl",
                split=f"train",
            )
