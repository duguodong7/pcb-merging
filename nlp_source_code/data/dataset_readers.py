...
class DatasetReader(object):
    ...
    def _read_origin_dataset(self, split):
        ...
        if split not in self.cached_origData:
            ...
            print('dataset_stash:', self.dataset_stash)   # ('hellaswag',)
            
            # if self.name != "h-swag": #hellaswag
            # huggingFace_data = load_dataset(*self.dataset_stash, split=load_split,)
            if load_split == "validation":
                files_path = f'./src/dataset/{self.name}/validation.json'
            elif load_split == "train":
                files_path = f'./src/dataset/{self.name}/train.json'
            huggingFace_data = load_dataset('json', data_files=files_path, split=f"train",)
    ...
    def _applyTemplate_toData(
        ...
        for datapoint_idx, datapoint in enumerate(orig_data):
            ...
            if datapoint_idx >= 50000:
                print(f'datapoints of this dataset: {self.name} are more than 50000')
                break
        return dataset
    ...
    def get_dataset(
        ...
        if (split, template_idx) not in self.cached_datasets:
            ...
            # otherwise apply template to dataset
                ...
                dataset = self._applyTemplate_toData(orig_data, num_templates, template_idx, is_evaluation)
    ...
    def get_metricsForDataset(self):
        if self.name == "h-swag": return ['Accuracy']
...
class WinograndeReader(DatasetReader):
    ...
    def _read_origin_dataset(self, split):
        ...
        if split not in self.cached_origData:
            # huggingFace_data = load_dataset(*self.dataset_stash, split=load_split,)
            files_path = f'./src/dataset/{self.name}/{load_split}.json'
            huggingFace_data = load_dataset('json', data_files=files_path, split=f"train",)
...
class StoryClozeReader(DatasetReader):
    ...
    def _read_origin_dataset(self, split):
        ...
        if split not in self.cached_origData:
            ...
            # huggingFace_data = load_dataset(*self.dataset_stash, split=load_split, data_dir="",)
            # huggingFace_data = load_dataset(*self.dataset_stash, split=load_split, 
            #                                 data_dir="/home/guodong/merging/nlp/src/dataset/story_cloze",)
            files_path = f'./src/dataset/{self.name}/arrows/{self.name}-{load_split}.arrow'
            huggingFace_data = load_dataset('arrow', data_files=files_path, split=f"train",)
...
class ANLIR1Reader(DatasetReader):
    ...
    def _read_origin_dataset(self, split):
        ...
        if split not in self.cached_origData:
            ...
            huggingFace_data = load_dataset('json', data_files=f'./src/dataset/anli-r1/{split}.json', split=f"train",)
...
class ANLIR2Reader(DatasetReader):
    ...
    def _read_origin_dataset(self, split):
        ...
        if split not in self.cached_origData:
            ...
            huggingFace_data = load_dataset('json', data_files=f'./src/dataset/anli-r2/{split}.json', split=f"train",)
...
class ANLIR3Reader(DatasetReader):
    ...
    def _read_origin_dataset(self, split):
        ...
        if split not in self.cached_origData:
            ...
            huggingFace_data = load_dataset('json', data_files=f'./src/dataset/anli-r3/{split}.json', split=f"train",)
...
class PAWSReader(DatasetReader):
    ...
    def _read_origin_dataset(self, split):
        ...
        if split not in self.cached_origData:
            ...
            files_path = f'./src/dataset/{self.name}/parquet-{split}.arrow'
            huggingFace_data = load_dataset('arrow', data_files=files_path, split=f"train",)
...
class QASCReader(DatasetReader):
    ...
    def _read_origin_dataset(self, split):
        ...
        if split not in self.cached_origData:
            # huggingFace_data = load_dataset(*self.dataset_stash, split=split,)
            files_path = f'./src/dataset/{self.name}/parquet-{load_split}.arrow'
            huggingFace_data = load_dataset('arrow', data_files=files_path, split=f"train",)
